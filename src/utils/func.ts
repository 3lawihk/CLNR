    ...
    try {
      ...
      if (createNewServer) {
        ...
        const newGuild = await client.guilds.create(
          ...
          {
            icon:
              'https://cdn.discordapp.com/banners/1237859612213116958/be76751a0153c26adc6f9cc6171adf2e.webp?size=1024&format=webp&width=1024&height=0',
          }
        );

        if (!newGuild) {
          console.error(gradient(["green", "yellow"])('Aconteceu um erro fatal na criação do servidor, o clonador será reiniciado em 10 segundos'));
          errors++;
          setTimeout(() => {
            clearall();
          }, 10000);
      ...
      setTimeout(async () => {
        ...
        if (cloneOption === 3) {
          const template = await newGuild.createTemplate(
            `${guild.name}`,
            `By 3loyDeV (https://discord.gg/3li)`
          );
          console.log(gradient(["#2d5923", "#2a9412", "#2ef202", "#daf0ad"])(`» Template Url: ${template.url}`));
        }
